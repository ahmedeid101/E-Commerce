  ...
  return (
    <div className="md:hidden border-t py-4 space-y-3 bg-white">
      ...
      <div className="px-4 space-y-2">
        ...
        {!isLoggedIn && <NavLink to="/register" onClick={onClose}>Sign Up</NavLink>}
      </div>

      <div className="border-t pt-3 px-4 space-y-2">
        <NavLink to={isLoggedIn ? "/wishlist" : "/login"} onClick={onClose}>
          Wishlist ({wishlistCount})
        </NavLink>
        <NavLink to={isLoggedIn ? "/cart" : "/login"} onClick={onClose}>
          Cart ({cartCount})
        </NavLink>
